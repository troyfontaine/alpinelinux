...
@test "ARM 3.8 package installs cleanly" {
  run docker container run --rm troyfontaine/armhf-alpinelinux:3.8 apk add --no-cache openssl
  [ $status -eq 0 ]
}

@test "ARM 3.8 timezone" {
  run docker container run --rm troyfontaine/armhf-alpinelinux:3.8 date +%Z
  [ $status -eq 0 ]
  [ "$output" = "UTC" ]
}

@test "ARM 3.8 repository list is correct" {
  run docker container run --rm troyfontaine/armhf-alpinelinux:3.8 cat /etc/apk/repositories
  [ $status -eq 0 ]
...
@test "ARM 3.8 root password is disabled" {
  run docker container run --rm --user nobody troyfontaine/armhf-alpinelinux:3.8 su
  [ $status -eq 1 ]
}
